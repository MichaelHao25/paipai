...
import { Dimensions, Image, StyleSheet, Text, TouchableHighlight, View, FlatList, RefreshControl, ActivityIndicator } from 'react-native';
...
export default class GameList extends Component {
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <CommonTitleBar nav={this.props.navigation} title={"充值记录"} color={"#686f78"} />
    ...
    _renderItem = (obj) => {
        const { fee, statusText, goodsname } = obj.item;
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 15, paddingTop: 10, paddingBottom: 10, alignItems: 'center', borderBottomColor: '#ccc', borderBottomWidth: 1 }}>
                <View>
                    <Text style={{ color: '#353535', fontSize: 16, paddingBottom: 10 }}>{goodsname}</Text>
                    <Text>{`￥${fee}`}</Text>
                </View>
                <View>
    ...
    fetchData = () => {
        StorageUtil.get('tokeninfo', (err, obj) => {
            if (err) {
                ...
                fetch(`http://app.daicui.net/user/rechargeRecords?pageNo=${page}&pageSize=20`, {
                    headers: {
