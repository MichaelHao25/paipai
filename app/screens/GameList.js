...
export default class GameList extends Component {
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <CommonTitleBar nav={this.props.navigation} title={"游戏记录"} color={"#686f78"} />
    ...
    _renderItem = (obj) => {
        const { createTime, desc1, inoutNum, money, roomName } = obj.item;
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 15, paddingTop: 10, paddingBottom: 10, alignItems: 'center', borderBottomColor: '#ccc', borderBottomWidth: 1/PixelRatio.get() }}>
                <View>
                    <Text style={{ color: '#353535', fontSize: 16, paddingBottom: 10 }}>{roomName}</Text>
                    <Text style={{ color: inoutNum >= 0 ? '#e4b247' : '#cd6654' }}>{`抢到：${money}(${desc1}) 盈亏：${inoutNum}元`}</Text>
                </View>
                <View>
                    <Text style={{ color: '#a1a1a1' }}>{createTime}</Text>
                </View>
            </View>
        )
    }
    ...
    fetchData = () => {
        StorageUtil.get('tokeninfo', (err, obj) => {
            if (err) {
                ...
                fetch(`http://app.daicui.net/user/myBonus?pageNo=${page}&pageSize=12`, {
                    headers: {
